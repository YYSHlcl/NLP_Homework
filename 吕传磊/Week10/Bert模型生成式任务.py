...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab):
        ...
        # 加载预训练的Bert模型
        self.bert = BertModel.from_pretrained(r"F:\人工智能NLP\NLP资料\week6 语言模型\bert-base-chinese", return_dict=False)
        # 分类层，将Bert输出映射到词汇表大小
    ...
    def forward(self, x, mask=None, y=None):
        ...
        # 使用Bert模型处理输入，并应用掩码
        if mask is not None:
            x, _ = self.bert(inputs_embeds=x, attention_mask=mask)
        else:
            x, _ = self.bert(inputs_embeds=x)
        # 应用Dropout层
        x = self.dropout(x)
        # 分类层进行预测
...
def build_sample(vocab, window_size, corpus):
    ...
    # 掩码序列的后半部分
    half_length = len(x) // 2
    mask = [1] * half_length + [0] * (len(x) - half_length)
    return x, y, mask
...
# 文本生成测试代码
# opennings 开头文本   model 模型   vocab 词表
...
if __name__ == "__main__":
    ...
    # 打开文件并读取内容
    with open('corpus.txt', 'r', encoding='ansi') as file:
        content = file.read()
    # 去除所有空白字符
    cleaned_content = content.replace(" ", "").replace("\t", "").replace("\n", "").replace(".", "")
    # 将处理后的内容写入新文件
    with open('corpus_test.txt', 'w', encoding='ansi') as output_file:
        output_file.write(cleaned_content)
    train("corpus_test.txt", False)
